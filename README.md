...
Finance view: [http://localhost:3001/finance](http://localhost:3001/finance). It reads confirmed booking revenue and driver cash directly from PostgreSQL, records fuel, driver-pay, amortisation, marketing, and other operational expenses, and shows net profit in the tenant's base currency. EUR, USD, PLN and UAH remain visible separately, while the owner-configured rates convert every available currency into the base-currency summary; missing rates are shown explicitly instead of silently dropping or mixing money. The route salary is copied to each departure and recorded once as a driver-pay expense when that trip is completed. The cash ledger also shows every driver's current outstanding balance across the entire cash history, so a manager can record a precise hand-in to the company cash desk.
...
Operational availability is expressed through configurable route, trip, driver and vehicle statuses. Existing legacy availability intervals remain enforced by the API for historical records, while the standalone screen redirects to route management.
...
In **Settings → Payment**, an owner or developer enters the merchant name, Ukrainian IBAN, EDRPOU/INN, bank name, MFO, bank EDRPOU and optional logo. No payment intermediary is used: funds go directly to the configured FOP account.

After a booking, the public checkout and bot offer **IBAN transfer** or **cash on boarding**. The direct-payment card shows the recipient, exact amount, purpose, NBU QR and a payment link, and polls the payment status without reloading the page. Bank transfer is available only in UAH; cash supports EUR, USD, PLN and UAH. A transfer holds the seats for 30 minutes. When a personal Monobank API token is configured, the bot checks the statement at the documented rate and confirms a transfer only when both the exact amount and unique `VIVAT-…` purpose match; it then confirms the booking and queues a Telegram receipt. Manual confirmation remains available to managers as a recovery path.

Regular routes can use either a fixed fare or a per-kilometre tariff by vehicle class. The route stores its distance, class-specific rates, currency and driver salary. The planner calculates the suggested trip price after both route and vehicle are selected, while preserving the dispatcher’s ability to adjust an individual departure.
