...
export default function Filtering8({ trips, timezone, seats, onSelect }: { trips: PublicTrip[]; timezone: string; seats: number; onSelect: (trip: PublicTrip) => void }) {
  ...
  }).sort((a, b) => sort === "price" ? a.currency.localeCompare(b.currency) || a.priceMinor - b.priceMinor : sort === "duration" ? (Date.parse(a.endsAt) - Date.parse(a.startsAt)) - (Date.parse(b.endsAt) - Date.parse(b.startsAt)) : Date.parse(a.startsAt) - Date.parse(b.startsAt)), [trips, timezone, departure, duration, sort])
  ...
  return <section aria-label="Результати пошуку" className="min-w-0">
    ...
    <div className="grid min-w-0 items-start gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10">
      <aside aria-label="Фільтри рейсів" className={`${filtersOpen ? "block" : "hidden"} space-y-6 rounded-[var(--rb-r-lg)] border border-border bg-card p-5 lg:block`}>
      ...
      <div className="min-w-0">
        {!filtered.length ? <div className="rounded-[var(--rb-r-lg)] border border-dashed border-border p-8 text-center sm:p-12"><Bus className="mx-auto mb-4 size-8 text-primary" /><h2 className="text-xl font-semibold">Рейсів не знайдено</h2><p className="mt-2 text-sm text-muted-foreground">Оберіть іншу дату чи напрямок або скиньте фільтри.</p><Button variant="outline" className="mt-5" onClick={reset}>Скинути фільтри</Button></div> : <ul className="space-y-3">
          {filtered.map((trip) => <li key={trip.id} className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-300"><article className="flex min-w-0 flex-col gap-5 rounded-[var(--rb-r-lg)] border border-border bg-card p-5 transition-colors hover:border-primary/40 sm:p-6 xl:flex-row xl:items-center">
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground xl:block xl:space-y-2"><p className="flex items-center gap-1.5"><Clock3 className="size-3.5" />{hours((Date.parse(trip.endsAt) - Date.parse(trip.startsAt)) / 60_000)}</p><p>Вільних місць: <span className="font-semibold text-foreground">{trip.availableSeats}</span></p></div>
            <div className="flex items-center justify-between gap-4 border-t border-border pt-4 xl:min-w-52 xl:border-0 xl:pt-0"><div><p className="text-xl font-bold tabular-nums text-primary">{travelMoney(trip.priceMinor, trip.currency)}</p><p className="mt-1 text-xs text-muted-foreground">{seats > 1 ? `${travelMoney(trip.priceMinor * seats, trip.currency)} за ${seats} місця` : "Оплата при посадці"}</p></div><Button className="h-11 px-4" onClick={() => onSelect(trip)}>Обрати<ArrowRight /></Button></div>
          </article></li>)}
        </ul>}
      </div>
    </div>
