...
export function DatePicker({ id, label, value, onValueChange, min, max, disabled, required, className, invalid, describedBy }: {
  id?: string; label: string; value: string; onValueChange: (value: string) => void; min?: string; max?: string; disabled?: boolean; required?: boolean; className?: string; invalid?: boolean; describedBy?: string
}) {
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    <PopoverTrigger render={<Button id={id} type="button" variant="outline" disabled={disabled} aria-label={label} aria-required={required} aria-invalid={invalid} aria-describedby={describedBy} className={cn("h-11 w-full justify-between rounded-xl bg-background px-3 font-normal text-foreground aria-expanded:bg-background dark:bg-background dark:hover:bg-muted", className)} />}>
      <span className="truncate">{selected ? new Intl.DateTimeFormat("ru-RU", { day: "2-digit", month: "2-digit", year: "numeric" }).format(selected) : "Выберите дату"}</span><CalendarDays className="size-4 shrink-0" />
    </PopoverTrigger>
