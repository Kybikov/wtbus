...
async function forward(method: "GET" | "PATCH", body?: unknown) {
  ...
  try {
    const response = await apiFetch(`${apiBaseURL}/api/v1/me/preferences`, {
      method,
      cache: "no-store",
      headers: body
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error },
        { status: [400, 401].includes(response.status) ? response.status : 502 }
      )
    }
    return NextResponse.json(payload)
  } catch {
    return NextResponse.json(
      { error: "Сервис настроек интерфейса временно недоступен." },
      { status: 503 }
    )
  } finally {
    clearTimeout(timer)
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Не удалось прочитать настройки интерфейса." },
      { status: 400 }
    )
  }
}
